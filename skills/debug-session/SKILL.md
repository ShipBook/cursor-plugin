...
1. Call `get-account-apps` to identify the app (returns name, appId, platform, key, integration). If the user didn't specify an app, detect the current project's platform and filter matching apps. If there are still multiple matches, ask the user which one.
